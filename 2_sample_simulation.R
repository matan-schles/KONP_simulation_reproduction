...
other_tests <- function(time,delta,trt)
{
  ...
  pv_list <- list(Pvalue_logrank = Pvalue_logrank, Pvalue_ppw = Pvalue_ppw,
                  pvalue_pf_pulled = pvalue_pf_pulled)
...
power_of_test <- function(iterations,imputations,permutations,n,scenario)
{
  ...
  res=foreach(core = 1:nr.sim.cores, .packages = c('KONPsurv',"survival","YPmodel","parallel"),
              ...
              .errorhandling ="pass" ) %dorng% {
                ...
                tab_usage <- c() #The average usage of tables for the non-permuted test statistic
                tab_usage_perm <- c() #The average usage of tables for the permuted test statistics
                ...
                for (j in 1:ceiling((iterations)/nr.sim.cores))
                {
                  ...
                  while ( inherits( d , "error")) # if yp_all had returned an error re-sample the data
                  {
                    ...
                    while(sum(delta[trt==1])<2 | sum(delta[trt==2])<2) #  #making sure we have at least 2 events in each grou
                    {
                      data <- (get(scenario))(n)
                ...
                return(data.frame(p_pearson=p_pearson, p_lr=p_lr,
                                  ...
                                  p_yp_perm=p_yp_perm,
                                  p_ppw=p_ppw, p_pf=p_pf,
                                  censorship_1=censorship_1,
                                  censorship_2=censorship_2,
  ...
  res_vec <- c(power_vec,mean(res$censorship_1), mean(res$censorship_2),
